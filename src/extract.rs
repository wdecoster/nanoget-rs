...
pub fn extract_metrics(args: &ExtractArgs) -> Result<MetricsCollection, NanogetError> {
    info!("Starting nanoget extraction with {} files", args.files.len());
    ...
    info!("Extraction complete: {} reads processed", combined.summary.read_count);
...
fn process_single_file(file: &Path, file_type: &FileType, args: &ExtractArgs) -> Result<MetricsCollection, NanogetError> {
...
fn process_fastq(file: &Path, rich: bool) -> Result<Vec<ReadMetrics>, NanogetError> {
    ...
    info!("Finished processing {} reads from {}", metrics.len(), file.display());
    Ok(metrics)
}
...
fn process_bam(file: &Path, keep_supplementary: bool) -> Result<Vec<ReadMetrics>, NanogetError> {
    ...
    for result in bam_reader.records() {
        ...
        let mapping_quality = if record.mapq() == 255 { None } else { Some(record.mapq()) };
        
        // Calculate quality scores
        let quality = record.qual().iter().any(|&q| q != 255).then(|| {
            utils::average_quality(record.qual()).unwrap_or(0.0)
        });
        ...
        let read_metrics = ReadMetrics::new(Some(read_id), length)
            ...
            .with_alignment(aligned_length, aligned_quality, mapping_quality, percent_identity);
...
fn process_summary(file: &Path, read_type: &str, barcoded: bool) -> Result<Vec<ReadMetrics>, NanogetError> {
    ...
    for result in csv_reader.records() {
        ...
        let (length_field, quality_field) = match read_type {
            ...
            _ => return Err(NanogetError::InvalidInput(format!("Unsupported read type: {}", read_type))),
        };
        
        let length: u32 = row.get(length_field)
        ...
        let quality: f64 = row.get(quality_field)
        ...
        let channel_id: Option<u16> = row.get("channel")
            .and_then(|s| s.parse().ok());
        
        let start_time = row.get("start_time")
            .and_then(|s| s.parse::<f64>().ok())
            .map(|timestamp| {
                use chrono::{Utc, TimeZone};
                Utc.timestamp_opt(timestamp as i64, (timestamp.fract() * 1e9) as u32).single()
            })
            .flatten();
...
fn parse_rich_fastq_metadata(desc: &str) -> Option<RichFastqMetadata> {
            ...
            match key {
                ...
                "start_time" => {
                    if let Ok(timestamp) = value.parse::<f64>() {
                        use chrono::{Utc, TimeZone};
                        metadata.start_time = Utc.timestamp_opt(timestamp as i64, 
                            (timestamp.fract() * 1e9) as u32).single();
                    }
                }
                "duration" => {
    ...
    if metadata.channel_id.is_some() || metadata.start_time.is_some() || 
       metadata.duration.is_some() || metadata.run_id.is_some() {
        Some(metadata)
    } else {
        None
    }
